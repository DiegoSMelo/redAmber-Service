...
public class CursoWS {
	...
	public String buscarCursoPorNomeESigla(@PathParam("nomeCurso") String nomeCurso,
			@PathParam("siglaCurso") String siglaCurso) {
	...
	@GET
	@Path("listar-cursos-por-tipo/{tipoCurso}")
	@Produces({ MediaType.APPLICATION_JSON, MediaType.TEXT_XML })
	public String listarCursosPorTipo(@PathParam("tipoCurso") String tipoCurso) {
		
		try {
			TipoCurso tipo = null;
			
			if (tipoCurso.equalsIgnoreCase("TECNICO") || tipoCurso.equalsIgnoreCase("Técnico")) {
				tipo = TipoCurso.TECNICO;
			}
			if (tipoCurso.equalsIgnoreCase("SUPERIOR") || tipoCurso.equalsIgnoreCase("Superior")) {
				tipo = TipoCurso.SUPERIOR;
			}
			
			List<Curso> cursos = this.rnCurso.listarCursosPorTipo(tipo);
			return this.gson.toJson(cursos);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
